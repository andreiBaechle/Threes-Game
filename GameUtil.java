...
{  
  ...
  public int getScoreForValue(int value)
  {
	 ...
	 int n = 0;
	  ...
	  {
			 n = (int) ((Math.log(value)/Math.log(2))-(Math.log(3)/Math.log(2)));
  ...
  public TilePosition generateRandomTilePosition(int[][] grid, Random rand, Direction lastMove)
  {
	 ...
	 if (lastMove.equals(Direction.DOWN))
	 {
		 int totalCol = grid[0].length;
		 int val = rand.nextInt(totalCol);
		 tile = new TilePosition(0, val, generateRandomTileValue(rand));
		 while(flag)
		 {
			 int rando = rand.nextInt(totalCol);
			 
			 if(grid[0][rando] == 0)
			 {
				 tile = new TilePosition( 0, rando ,generateRandomTileValue(rand));
				 flag = false;
			 }
		 }
	 }
	 else if (lastMove.equals(Direction.UP))
     {
		 int totalRow = grid.length;
		 int totalCol = grid[0].length;
		 int val = rand.nextInt(totalCol);
		 tile = new TilePosition(totalRow, val, generateRandomTileValue(rand));
		 while(flag)
		 {
			 int rando = rand.nextInt(totalCol);
			 
			 if(grid[totalRow-1][rando] == 0)
			 {
				 tile = new TilePosition(totalRow-1, rando ,generateRandomTileValue(rand));
				 flag = false;
			 }
		 }
	 }
	 else if (lastMove.equals(Direction.LEFT))
	 {
		int totalRow = grid.length;
		int totalCol = grid[0].length;
		int val = rand.nextInt(totalRow);
		 tile = new TilePosition(val, totalCol -1  , generateRandomTileValue(rand));
		 
		 while(flag)
		 {
			 int rando = rand.nextInt(totalRow);
			 
			 if(grid[rando][totalCol-1] == 0)
			 {
				 tile = new TilePosition(rando, totalCol-1 ,generateRandomTileValue(rand));
				 flag = false;
			 }
		 }
	 }
	 else if (lastMove.equals(Direction.RIGHT))
	 {
		 int totalRow = grid.length;
	     int val = rand.nextInt(totalRow);
	     tile = new TilePosition(val, 0 , generateRandomTileValue(rand));
	     while(flag)
		 {
			 int rando = rand.nextInt(totalRow);
			 ...
			 if(grid[rando][0] == 0)
			 {
				 tile = new TilePosition(rando, 0 ,generateRandomTileValue(rand));
				 flag = false;
			 }
		 }
	 }
  ...
  public ArrayList<Move> shiftArray(int[] arr)
  {
	 ArrayList<Move>list = new ArrayList<Move>();
		 ...
		 for (int i = 0; i < arr.length -1; i++)
		 {
			 ...
			 else if (mergeValues(arr[i],arr[i+1]) != 0)
	         {
			   int newVal = mergeValues(arr[i],arr[i+1]);
			   arr[i+1] = 0;
			   arr[i] = newVal;
		       list.add(new Move( i + 1 , i , arr[i + 1] , arr[i], newVal));
